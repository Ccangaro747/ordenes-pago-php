...
$sql = "SELECT * FROM orn_ordenes 
            WHERE fech_op >= '" . $_SESSION["desde"] . "' AND 
                  ...
                  importe_total <= '" . $_SESSION["importehasta"] . "'  AND jurisdiccion = '" . $_SESSION["entidad"] . "'";
...
<body>
            ...
            <ul class="navbar-nav ml-auto">
                <li class="nav-item dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="usuarioDropdown">
                        <a class="dropdown-item" href="cambiar-contrasena.php">Cambiar Contraseña</a>
    ...
    <div id="container-form">
        <h2>
            <?php echo $_SESSION["entinom"]; ?>
        </h2>
        <form id="filterForm" action="procesar_filtro.php" method="post">
            <label for="startDate" id="startDate-container">Fecha desde:
                <input type="date" id="startDate" name="desde" value="<?php echo $_SESSION['desde']; ?>"
                    required /></label>
            <label for="endDate" id="endDate-container">Fecha hasta: <input type="date" id="endDate" name="hasta"
                    value="<?php echo $_SESSION['hasta']; ?>" required /></label>
            <label for="amountFrom">Importe desde:
                <input type="number" id="amountFrom" name="importedesde" min="0"
                    value="<?php echo $_SESSION['importedesde']; ?>" /></label>
            <label for="amountTo">Importe hasta:
                <input type="number" id="amountTo" name="importehasta" min="0"
                    value="<?php echo $_SESSION['importehasta']; ?>" /></label>
            <label for="provider">Proveedor:
                <select id="provider" name="proveedor">
                    ...
                    while ($prov = mysqli_fetch_array($proveedores)) {
                        ...
                        <option value="<?php echo $prov['cod_prov']; ?>" <?php // Aca me fijo si el nro de proveedor coincide con el proveedor del filtro, lo muestro como seleccionado
                ...
                <table id="datatable_users" class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th class="centered">Jurisdicción</th>
                    ...
                    <tbody>
                        ...
                        while ($orden = mysqli_fetch_array($ordenes)) { ?>
                            <tr>
                                <td>
                                    <?php echo $orden['nro_op']; ?>
                                </td>
                                <td>
                                    <?php echo $orden['jurisdiccion']; ?>
                                </td>
                                <td>
                                    <?php echo $orden['fech_op']; ?>
                                </td>
                                <td>
                                    <?php echo $orden['razon_social']; ?>
                                </td>
                                <td>
                                    <?php echo $orden['importe_total']; ?>
                                </td>
                                <td>
                                    <?php echo $orden['concepto']; ?>
                                </td>
